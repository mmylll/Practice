...
int bitCount(int x) {
    /*
       ...
       右移的作用就是让加法运算的相同数位对齐。
   */
    int num_1 = (((0x55<<8)+0x55)<<8)+0x55;
    int num_2 = (((0x33<<8)+0x33)<<8)+0x33;
    int num_3 = (((((0xf0<<8)+0xf0)<<8)+0xf0)<<4)+0xf;
    int num_4 = (0xff<<16)+0xff;
    int num_5 = (0xff>>8)+0xff;
    x= (x & num_1 )+ ((x >> 1) & num_1);
    x= (x & num_2) + ((x >> 2) & num_2);
    x= (x & num_3) + ((x >> 4) & num_3);
    x= (x & num_4) + ((x >> 8) & num_4);
    x= (x & num_5) + ((x >> 16) & num_5);
    return x;
}
/* 
