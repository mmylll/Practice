...
void SetMap()
{
	for (int i = 0; i < 13; i++)
	{
		for (int j = 0; j < 13; j++)
		{
			...
			case 97:
			case 98:
				putimage(j * 60, i * 60, &Ladder);         //上楼梯子和下楼梯子
				break;
			case 99:
...
void playGame()
{
	while (1)
	{
		...
		case 'q':
		case 'Q':
			ID = MessageBox(hwnd, "保存游戏", "是否保存游戏？", MB_YESNO);
			if (ID == IDYES)
				SaveGame();
			break;
		case 'w':
		case 'W':
		case 72:
			if (map[playerh][playerx - 1][playery] == 1) {         //下一步是地板
				map[playerh][playerx - 1][playery] = 99;
				map[playerh][playerx][playery] = 1;
				playerx--;
			}
			else if (map[playerh][playerx - 1][playery] == 6) {      //下一步是蓝钥匙
				player.Num_Blue_Key++;
				map[playerh][playerx - 1][playery] = 99;
				map[playerh][playerx][playery] = 1;
				playerx--;
			}
			else if (map[playerh][playerx - 1][playery] == 7) {     //下一步是黄钥匙
				player.Num_Yellow_Key++;
				map[playerh][playerx - 1][playery] = 99;
				map[playerh][playerx][playery] = 1;
				playerx--;
			...
			//红蓝药水
				...
				if (map[playerh][playerx - 1][playery] == 8)
					player.HP += 200;   //红色药水加200hp
				else
					player.HP += 500;  //蓝色药水加500hp
				map[playerh][playerx - 1][playery] = 99;
				map[playerh][playerx][playery] = 1;
				playerx--;
			}
		...
		case 'a':
		case 75:
			if (map[playerh][playerx][playery - 1] == 1) {         //下一步是地板
				map[playerh][playerx][playery - 1] = 99;
				map[playerh][playerx][playery] = 1;
				playery--;
			}
			else if (map[playerh][playerx][playery - 1] == 6) {      //下一步是蓝钥匙
				player.Num_Blue_Key++;
				map[playerh][playerx][playery - 1] = 99;
				map[playerh][playerx][playery] = 1;
				playery--;
			}
			else if (map[playerh][playerx][playery - 1] == 7) {     //下一步是黄钥匙
				player.Num_Yellow_Key++;
				map[playerh][playerx][playery - 1] = 99;
				map[playerh][playerx][playery] = 1;
				playery--;
